...
这个项目使用 Vite + React + TypeScript 构建一个网页端可游览的三体语境下太阳系 3D 模型。当前代码实现的是第一阶段：三体人到来前的太阳系高性能 3D 场景。
...
- 天体数据：`src/data/solarSystem.ts` 是核心数据源，包含太阳、八大行星、主要卫星、矮行星、小天体族群，以及第二阶段占位的 `futureExternalLayers`。
...
- 不要把自然天体、第二阶段人造结构和 UI 状态混在同一层抽象里。三体舰队、水滴、地球舰队、太空电梯应作为独立外部层或独立配置接入，并注册可选择/可聚焦目标。
